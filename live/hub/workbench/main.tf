...
# ssh_ingress_cidrs를 물려받는다. 의도된 트레이드오프다(서브넷 자체가 "관리 전용"
# 용도로 분리돼 있어 워크로드 트래픽과 섞이지 않는다). 우선순위 100-199는 이 root가
# 예약한다. live/hub/networking이나 다른 소비자가 이 NSG에 규칙을 더 얹을 땐 이 범위를
# 피해야 충돌(우선순위 중복은 apply 시점 Azure API 에러)이 없다.
...
# hub AKS 클러스터를 조회한다(같은 RG. terraform_remote_state 대신 data).
...
module "aks_workbench" {
  ...
  # 최신값은 az vm image list --location koreacentral --publisher Canonical
  # --offer ubuntu-24_04-lts --sku server --all 로 조회한다.
