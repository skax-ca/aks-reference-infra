...
#
# live/hub/networking을 템플릿으로 복사해 dev 구독 값으로 바꾼 것이다. 구조는
# 동일하고 CIDR·env만 다르다.
#
...
locals {
  ...
  # (live/hub/networking/main.tf가 예약한 값).
  vnet_cidr = "10.61.0.0/16"

  # ⛔ Pod 전용 secondary CIDR을 두지 않는다. Overlay CNI라 Pod IP가 VNet 밖 오버레이
  #    대역(모듈의 pod_cidr)에서 나온다. 근거는 live/hub/networking/main.tf의 locals
  #    주석(반복하지 않는다).
...
module "vnet" {
  ...
  # 사람이 선생성한 워크로드 RG(bootstrap.sh, BOOTSTRAP_TARGET=spoke SPOKE_ENV=dev가
  # 만든다). 이 root 가 RG 를 만들지 않는다.
  ...
  subnet_groups = {
    ...
    # AKS 노드 자리(live/dev/aks가 소비). Pod IP는 여기서 뜨지 않는다. Overlay CNI라
    # VNet 밖 오버레이 CIDR에서 받는다(hub와 동일 근거, live/hub/networking/main.tf 참고).
    "aks-node" = {
  ...
  # hub와 동일하게 true가 기본이다. AWS 원본의 dev는 false(파기가 잦은 환경 가정)지만,
  # 이 레퍼런스 인스턴스는 실수 삭제 방지를 우선한다. 해제하려면
  # deletion_protection=false로 먼저 apply한 뒤 destroy(hub와 동일 2단계 절차).
  # ⚠️ 지금 false인 것은 철거→재구축 중이라서다. 재구축 후 true로 되돌린다
  #    (docs/spoke-lifecycle.md 철거 절).
