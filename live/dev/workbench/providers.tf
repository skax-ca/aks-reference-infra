...
#
# live/dev/aks/providers.tf와 완전히 동일한 근거(같은 dev 구독, 같은 CI 신원) — 이 root
# 고유의 변경 사유가 없어 그대로 복제한다(azapi는 이 root가 쓰지 않아 선언하지 않는다).

provider "azurerm" {
  subscription_id = var.subscription_id

  # live/dev/networking·live/dev/aks와 동일 근거로 자동 등록을 끈다: CI 신원은
  # 이 배포와 무관한 프로바이더까지 plan 시작 시 등록하려 시도해 멈추는 걸 막는다
  # (2026-08-27 hub 첫 CI plan 실측, live/hub/workbench/providers.tf 참고).
...
# ── CI 전용 인증 가드 ─────────────────────────────────────────────────────────
#
# live/dev/aks/providers.tf와 동일한 가드 — 로컬 az login으로도 apply가 성립해버리는 것을
