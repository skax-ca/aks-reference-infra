...
variable "require_oidc" {
  description = <<-EOT
    ...
    차단). live/dev/aks/variables.tf와 동일 근거.
  EOT
  type        = bool
  default     = true
  nullable    = false
...
variable "ssh_ingress_cidrs" {
  description = <<-EOT
    ...
    CI   : GitHub repo 변수 AZURE_DEV_WORKBENCH_SSH_CIDRS(JSON 배열 문자열, 예: ["1.2.3.4/32"])
...
variable "admin_login_principal_id" {
  description = <<-EOT
    ...
    CI   : GitHub repo 변수 AZURE_DEV_WORKBENCH_ADMIN_OBJECT_ID
