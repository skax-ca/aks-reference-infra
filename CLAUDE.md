...
| `aks-platform-gitops` | ArgoCD Application·AppProject·cluster-secret (계층 2) | ✅ hub 클러스터에 배포 완료(self-managed ArgoCD·AGFC·Karpenter·Kyverno, `eks-platform-gitops` 대응). 설계 근거는 `docs/decisions/aks-platform-gitops-*.md` |
...
`spoke_connections`, 근거는 `docs/decisions/live-hub-vwan-dev-networking.md`).
...
| 자격증명 | GitHub OIDC → 단일 App Registration, 구독 전체 스코프 `Owner`(2026-09-04 결정, AWS 원본의 `AdministratorAccess` 실행 Role과 스코프 축에서 완전 대칭. ✅ hub·dev 양쪽 재부트스트랩 완료). 방어선은 권한 크기가 아니라 FIC subject 하나로 좁힌 도달 경로뿐이다(아래 ⛔ 참고). 이전 설계(RG 스코프 커스텀 역할 + 6종 불변식)에서 왜 바뀌었는지는 `bootstrap/README.md`·`docs/decisions/bootstrap-credential-design.md` 참고 |
