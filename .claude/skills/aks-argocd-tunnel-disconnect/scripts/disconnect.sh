#!/usr/bin/env bash
# aks-argocd-tunnel-connect 로 연 hub ArgoCD 터널을 해제한다. 로컬 watchdog(ssh -L)·원격
# kubectl port-forward watchdog 을 전부 정리한다.
#
# eks-reference-infra의 eks-argocd-tunnel-disconnect를 대조해 포팅. SSM 세션 대신 SSH
# 프로세스를 정리한다는 점만 다르다.
set -uo pipefail
...
# connect.sh가 쓴 상태 폴더를 그대로 읽는다(짝 스킬 aks-argocd-tunnel-connect 밑의 .state/,
# 이 위치를 쓰는 이유는 connect.sh 쪽 주석 참고).
STATE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/aks-argocd-tunnel-connect/.state"
...
# (aks-argocd-tunnel-connect의 REMOTE_CMD와 동일 패턴).
#
