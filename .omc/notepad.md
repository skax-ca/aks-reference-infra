...
2026-08-27(3차 세션) - live/hub/networking hub 구독에 실제 apply 완료(GitHub Actions CI). repo skax-ca/aks-reference-infra 신설. 다음: live/hub/vwan 착수 또는 dev 구독 확보 후 live/dev/networking. project memory의 architecture 노트에 CI 재발 가능 버그 3건(FIC ID 접미사·azurerm 프로바이더 자동등록·getenv 미존재) 기록됨 - 새 live/* 루트 만들 때 먼저 확인.
